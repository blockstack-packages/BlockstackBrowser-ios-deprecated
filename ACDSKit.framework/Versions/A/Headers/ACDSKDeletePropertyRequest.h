...
/**
 Request to delete a single property (key-value pair) from a node. App must have write access for the property. A successful request results in an empty response.
 
 @see ACDSKEmptyResponse
 @see ACDSKAddPropertyRequest
 @see ACDSKNode
 */
@interface ACDSKDeletePropertyRequest : ACDSKRequest

/**
 (required) The ID of the node from which the property should be deleted.
 */
